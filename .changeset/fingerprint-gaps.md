...
- **tsconfig `paths` / `baseUrl`** — bare specifiers are mapped through the nearest `tsconfig.json` (walking up from each file, following `extends` across packages) with TypeScript's longest-prefix / `*` capture / multi-target semantics. Configs are parsed once per cache. Disable with `tsconfig: false` / `--no-tsconfig`.
